...
概率分布
---
方差：variance = 1/C * pow(x1-xavg, 2) + ... + pow(xn-xavg, 2)
标准差： standard deviation = sqrt(variance)

如果统计的数据是全部数据  C=N ，即总数，而如果统计的数据是一部分样本，则 C=N-1，少1用于修补数据。

在正态分布中，距平均值小于一个标准差之内的数值范围所占比率为全部数值之68%，根据正态分布，两个标准差之内的比率合起来为95%；三个标准差之内的比率合起来为99%。